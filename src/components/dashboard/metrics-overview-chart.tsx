...
export function MetricsOverviewChart({
  data: initialData,
}: MetricsOverviewChartProps) {
  ...
  const maxClientes = Math.max(
    ...processedData.map((item) => item.clientes || 0),
  );
  const maxIngresos = Math.max(
    ...processedData.map((item) => item.ingresos || 0),
  );
      ...
      <CardContent className="p-4">
        <Tabs defaultValue="combined" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="clientes">Alumnos</TabsTrigger>
