...
export function WorkingHours({
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `No se pudo actualizar ${dayLabel}: ${errorData.error || response.statusText}`,
          variant: "destructive",
        });
  ...
  const handleToggleDay = (dayId: string) => {
    ...
    if (!currentDaySchedule || typeof currentDaySchedule.id === "undefined") {
      toast({
        ...
        description: `No hay un horario base para ${getDayLabel(dayId)} en la base de datos. Esta acción requiere crear un nuevo horario.`,
        variant: "destructive",
      });
      return;
    }
  ...
  const handleTimeChange = (
    ...
    if (!currentDaySchedule || typeof currentDaySchedule.id === "undefined") {
      toast({
        title: "Acción no permitida",
        description: `No hay un horario base para ${getDayLabel(dayId)} en la base de datos.`,
        variant: "destructive",
      });
      return;
    }
